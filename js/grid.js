...
function buildYesterdayHeadshot(contestant, name) {
  const headshot = document.createElement("img");
  headshot.className = "yesterday-answer-headshot";
...
function showYesterdayAnswersPopup(row, col) {
  ...
  answers.forEach(answer => {
    ...
    item.appendChild(buildYesterdayHeadshot(contestant, answer?.name || ""));

    const nameText = document.createElement("span");
    nameText.className = "modal-list-name yesterday-answers-modal-name";
    ...
    const rarity = getYesterdayIntersectionRarity(contestant, data?.rows?.[row], data?.cols?.[col]);
    if (rarity !== null) {
      const rarityEl = document.createElement("span");
      rarityEl.className = "yesterday-answer-rarity";
      rarityEl.textContent = `${rarity}% all-time`;
      rarityEl.title = "Share of all guesses this intersection has ever drawn";
...
/**
 * How often this castaway is picked at this intersection across all time, as a
 * share of every guess the pairing has ever drawn.
 *
 * Carried over from the list this recap replaces: it is the counterweight to
 * yesterday's share, separating "the obvious answer here" from "yesterday's
 * crowd happened to converge". Pairings with almost no history are skipped
 * rather than reported as a percentage of nothing.
 */
function getYesterdayIntersectionRarity(contestant, rowCategoryId, colCategoryId) {
...
function buildYesterdayCell(data, row, col, contestantByNormalizedName) {
  ...
  cell.append(
    shareEl,
    zoomEl,
    buildYesterdayHeadshot(resolveAnswerContestant(top, contestantByNormalizedName), top?.name || ""),
    nameEl
  );
  ...
  cell.setAttribute(
    "aria-label",
    `${rowLabel} × ${colLabel}: top answer ${top?.name || ""}, ` +
