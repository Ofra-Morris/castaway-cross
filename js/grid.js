...
/**
 * The puzzle day before the one currently live, which is the day the recap is
 * supposed to be describing. Derived from the same 06:00 ET clock the board
 * runs on rather than from UTC, so the check below compares like with like.
 */
function getExpectedYesterdayKey() {
  if (!DAILY_PUZZLE_KEY) return null;
  const previous = new Date(`${DAILY_PUZZLE_KEY}T00:00:00Z`);
  previous.setUTCDate(previous.getUTCDate() - 1);
  return previous.toISOString().slice(0, 10);
}

/**
 * Resolve the castaway behind a recorded answer, for the headshot.
 *
 * The build already did the hard part: it has the roster and the category
 * definitions, so it settles each answer to a castaway id there -- by unique
 * name, or by which candidate is eligible for both of the cell's categories --
 * and labels it with a name that belongs to one person. An answer that arrives
 * with no id is one the build could not settle (both Russells fit the cell, and
 * the guess was typed rather than picked), so this does not second-guess it:
 * the caller gets null and draws the filler silhouette.
 *
 * The name lookup is only for files built before answers carried ids.
 */
function resolveAnswerContestant(answer, contestantByNormalizedName) {
  const castawayId = typeof answer?.castawayId === "string" ? answer.castawayId.trim() : "";
  if (castawayId) {
    const byId = getContestantByCastawayId(castawayId);
    if (byId) return byId;
  }

  const normalized = normalizeRarityName(answer?.name);
  if (!normalized) return null;

  const ids = NAME_TO_CASTAWAY_IDS.get(normalized);
  if (ids && ids.size > 1) return null;

  return contestantByNormalizedName.get(normalized) || null;
}

let CONTESTANT_BY_CASTAWAY_ID = null;

function getContestantByCastawayId(castawayId) {
  if (!CONTESTANT_BY_CASTAWAY_ID || CONTESTANT_BY_CASTAWAY_ID.size !== contestants.length) {
    CONTESTANT_BY_CASTAWAY_ID = new Map(contestants.map(c => [c.castaway_id, c]));
  }
  return CONTESTANT_BY_CASTAWAY_ID.get(castawayId) || null;
}

function buildAnswerHeadshot(contestant, name, className = "answer-avatar-photo") {
  const headshot = document.createElement("img");
  headshot.className = className;
  headshot.loading = "lazy";
  headshot.decoding = "async";
  headshot.alt = "";
  headshot.src = contestant
    ? `/assets/headshots/${contestant.castaway_id}.webp`
    : "/assets/headshots/filler.webp";
  headshot.onerror = () => { headshot.src = "/assets/headshots/filler.webp"; };

  // A silhouette otherwise reads as a missing image rather than as the real
  // answer, which is that this guess was typed rather than picked and more
  // than one castaway answers to the name.
  headshot.title = contestant
    ? getCanonicalName(contestant)
    : `${name} — more than one castaway goes by this name`;

  return headshot;
}

/**
 * Every answer a cell drew yesterday, ranked, with each one's share of the
 * cell's guesses and whether it counted.
 *
 * This is the fuller list behind the magnifying glass: the tile can only show
 * the answer that won the cell, and on a cell where the guessing spread out
 * that one name is often a minority of the votes.
 */
function showYesterdayAnswersPopup(row, col) {
  ...
  const answers = getYesterdayCellAnswers(cellData);
  const totalGuesses = Number(cellData?.totalGuesses) || 0;

  const rowLabel = getCategoryById(data?.rows?.[row])?.label || "";
  const colLabel = getCategoryById(data?.cols?.[col])?.label || "";

  const modal = document.createElement("div");
  modal.className = "modal-overlay";

  const modalContent = document.createElement("div");
  modalContent.className = "modal modal-yesterday-answers";
  modalContent.setAttribute("role", "dialog");
  modalContent.setAttribute("aria-modal", "true");
  modalContent.setAttribute("aria-labelledby", "yesterday-answers-title");

  const title = document.createElement("h3");
  title.id = "yesterday-answers-title";
  title.textContent = rowLabel && colLabel ? `${rowLabel} × ${colLabel}` : "Yesterday's Answers";

  const subtitle = document.createElement("p");
  subtitle.className = "modal-subtitle";
  subtitle.textContent = totalGuesses === 1
    ? "1 guess yesterday"
    : `${totalGuesses.toLocaleString()} guesses yesterday`;

  const modalList = document.createElement("div");
  modalList.className = "modal-list yesterday-answers-modal-list";

  const contestantByNormalizedName = buildContestantNameIndex();

  answers.forEach(answer => {
    const count = Number(answer?.count) || 0;
    const correctCount = Number(answer?.correctCount) || 0;
    // A name shared by two castaways can be right for some guessers and wrong
    // for others, so the row is coloured by which way most of them went and
    // the split itself is spelled out beside it.
    const wasCorrect = correctCount * 2 >= count && correctCount > 0;
    const isSplit = correctCount > 0 && correctCount < count;

    const item = document.createElement("div");
    item.className = `modal-list-item yesterday-answers-modal-item ${wasCorrect ? "is-correct" : "is-incorrect"}`;

    const contestant = resolveAnswerContestant(answer, contestantByNormalizedName);
    // Full names here and short ones on the tiles, matching the valid-answers
    // popup and the postgame grid. The stored name is only a fallback for an
    // answer no castaway could be resolved for.
    const nameEl = appendAnswerIdentity(
      item,
      contestant,
      getYesterdayAnswerFullName(contestant, answer)
    );
    nameEl.classList.add("yesterday-answers-modal-name");

    if (isSplit) {
      const splitEl = document.createElement("span");
      splitEl.className = "yesterday-answers-modal-split";
      splitEl.textContent = `${correctCount}/${count} valid`;
      splitEl.title = "This name matches more than one castaway";
      item.appendChild(splitEl);
    }

    const shareEl = document.createElement("span");
    shareEl.className = "yesterday-answers-modal-share";
    shareEl.textContent = totalGuesses > 0 ? `${formatGuessShare(count, totalGuesses)}%` : `${count}`;
    item.appendChild(shareEl);

    modalList.appendChild(item);
  });

  const children = [title, subtitle, modalList];

  const other = cellData?.otherAnswers;
  if (other && Number(other.answerCount) > 0) {
    const footnote = document.createElement("p");
    footnote.className = "modal-subtitle";
    footnote.textContent =
      `+ ${Number(other.answerCount).toLocaleString()} more answers ` +
      `(${Number(other.guessCount).toLocaleString()} guesses) below the cutoff.`;
    children.push(footnote);
  }

  if (!answers.length) {
    modalList.textContent = "No answers recorded for this cell.";
  }

  mountModalDialog(modal, modalContent, children);
}

/**
 * A headshot with the castaway's edition flag set into its lower-right rim.
 *
 * The flag used to sit beside the name as its own chip. Folding it onto the
 * face costs the row nothing and drops it from three elements to two, and the
 * tiles -- which never had a flag -- get one for free. The ring is the row's
 * own background rather than white, so the badge reads as a cut-out in the
 * photo instead of a sticker on top of it.
 */
function buildAnswerAvatar(contestant, name, sizeClass = "answer-avatar-list") {
  const wrap = document.createElement("span");
  wrap.className = `answer-avatar ${sizeClass}`;
  wrap.appendChild(buildAnswerHeadshot(contestant, name, "answer-avatar-photo"));

  const badge = contestant ? getEditionBadgeConfig(contestant) : null;
  if (badge) {
    wrap.appendChild(createEditionFlagBadge(badge));
  }
...
/**
 * The castaway's page on Survivor Reference, when they have one.
 *
 * Only the US edition is covered there, and the slug is the id plus the full
 * name. Module scope rather than a closure because both answer lists link out.
 */
function buildSurvivorReferenceUrl(contestant) {
...
/**
 * The leading half of a row in either answer list: face, edition flag, and the
 * name linked to Survivor Reference.
 *
 * Both lists name the same kind of thing, so they are built by the same code
 * rather than drifting apart. A castaway that could not be resolved -- which
 * only happens in yesterday's recap, for a guess typed rather than picked --
 * still gets a row, with the filler silhouette and no flag or link.
 */
function appendAnswerIdentity(row, contestant, name, { sizeClass = "answer-avatar-list" } = {}) {
  row.appendChild(buildAnswerAvatar(contestant, name, sizeClass));

  const url = contestant ? buildSurvivorReferenceUrl(contestant) : null;
  const nameEl = document.createElement(url ? "a" : "span");
  nameEl.className = url ? "modal-list-link modal-list-name" : "modal-list-name";
...
/**
 * The short name for a tile, and the full name for a list.
 *
 * The board already draws this distinction -- a postgame cell reads "Russell"
 * where the valid-answers popup reads "Russell Hantz" -- so the recap follows
 * it rather than inventing a third convention. The name recorded in the file is
 * the fallback for an answer that resolved to no castaway.
 */
function getYesterdayAnswerDisplayName(contestant, answer) {
  return contestant ? getCanonicalName(contestant) : (answer?.name || "");
}

function getYesterdayAnswerFullName(contestant, answer) {
  if (!contestant) return answer?.name || "";
  return contestant.name || contestant.display_name?.[0] || answer?.name || "";
}

/** Name → contestant, for resolving recorded answers back to a castaway. */
function buildContestantNameIndex() {
  const index = new Map();
...
/**
 * A cell's answers, ranked. Sorted here rather than trusting the file so the
 * tile's top answer and the list behind it can never disagree.
 */
function getYesterdayCellAnswers(cellData) {
  const answers = Array.isArray(cellData?.topAnswers) ? cellData.topAnswers : [];
  return [...answers].sort(
...
/**
 * A guess share, kept to one decimal below 10% so the long tail of a
 * spread-out cell does not collapse into a row of identical "1%"s.
 */
function formatGuessShare(count, total) {
  if (!(total > 0)) return "0";
  const pct = (count / total) * 100;
  return pct < 9.95 ? (Math.round(pct * 10) / 10).toString() : Math.round(pct).toString();
}

/**
 * Keep the recap in step with the board on screen.
 *
 * Yesterday's answers describe yesterday's grid, so they have nothing to do
 * with whether today's is finished -- the section belongs under the board from
 * the moment a daily puzzle is set up, not only once it has been submitted.
 * Outside daily mode it is removed rather than hidden, so a random or custom
 * board cannot leave the previous daily's recap sitting underneath it.
 */
function syncYesterdayPopularAnswers() {
  if (CURRENT_PUZZLE_MODE !== "daily") {
    document.getElementById("yesterday-answers")?.remove();
    return;
  }
...
function renderYesterdayPopularAnswers() {
  ...
  // The section is rendered when the board is set up and again when it is
  // submitted, so a player who opened the dropdown mid-puzzle would otherwise
  // find it collapsed again on the results screen.
  const wasOpen = container.querySelector(".yesterday-answers-details")?.open === true;
  container.innerHTML = "";

  const data = YESTERDAY_GRID_DATA;
  const expectedDate = getExpectedYesterdayKey();

  const hasAnyAnswers = !!data?.cells && Object.values(data.cells)
    .some(cell => Array.isArray(cell?.topAnswers) && cell.topAnswers.length > 0);

  // The stats are baked into a file that a scheduled job republishes a few
  // minutes after the 06:00 ET rollover, so there is a short window each
  // morning where the newest file still describes the day before yesterday.
  // Rendering it would put a grid of real-looking faces under a heading that
  // claims to be yesterday's, which is worse than admitting the numbers are not
  // in yet. The window is small but it is not zero, and a delayed job widens
  // it.
  // A payload with no puzzleDate at all cannot be shown to be yesterday's
  // either, so it takes the same path rather than rendering under a blank date.
  const isStale = !!expectedDate && data?.puzzleDate !== expectedDate;

  if (!data || data.noData || !data.cells || !hasAnyAnswers || isStale) {
    renderYesterdayPending(container);
    return;
  }
  ...
  summary.textContent = "Yesterday's Most Popular Answers";
  ...
  for (let row = 0; row < 3; row++) {
    ...
    for (let col = 0; col < 3; col++) {
      grid.appendChild(buildYesterdayCell(data, row, col, contestantByNormalizedName));
    }
  }
...
/**
 * One cell of the recap, built to read as a postgame cell: the answer that won
 * the cell, its face, its share of the guesses, green if it counted and red if
 * it did not, and the magnifying glass that opens everything else that was
 * guessed there.
 */
function buildYesterdayCell(data, row, col, contestantByNormalizedName) {
  const cellData = data.cells[`${row},${col}`];
  const answers = getYesterdayCellAnswers(cellData);
  ...
  const correctCount = Number(top?.correctCount) || 0;
  const wasCorrect = correctCount * 2 >= count && correctCount > 0;

  cell.classList.add(wasCorrect ? "is-correct" : "is-incorrect");

  const distinctAnswers = answers.length + Number(cellData?.otherAnswers?.answerCount ?? 0);

  const shareEl = document.createElement("div");
  shareEl.className = "yesterday-answer-share";
  shareEl.textContent = totalGuesses > 0 ? `${formatGuessShare(count, totalGuesses)}%` : "";
  shareEl.title = `${count} of ${totalGuesses} guesses`;

  const zoomEl = document.createElement("div");
  zoomEl.className = "yesterday-answer-zoom";
  zoomEl.setAttribute("aria-hidden", "true");
  zoomEl.innerHTML = `🔍 <span></span>`;
  zoomEl.querySelector("span").textContent = String(distinctAnswers);

  const contestant = resolveAnswerContestant(top, contestantByNormalizedName);
  const displayName = getYesterdayAnswerDisplayName(contestant, top);
  ...
  cell.append(
    shareEl,
    zoomEl,
    buildAnswerAvatar(contestant, top?.name || "", "answer-avatar-tile"),
    nameEl
  );
  ...
  cell.setAttribute(
    "aria-label",
    `${rowLabel} × ${colLabel}: top answer ${displayName}, ` +
    `${wasCorrect ? "valid" : "not valid"}, ` +
    `${formatGuessShare(count, totalGuesses)} percent of guesses. ` +
    `${distinctAnswers} answers guessed. Activate to see all of them.`
  );

  const open = () => showYesterdayAnswersPopup(row, col);
...
/**
 * Shown in place of the recap while the numbers are still being aggregated,
 * so the section's absence each morning reads as "not yet" rather than as the
 * feature having quietly broken.
 */
function renderYesterdayPending(container) {
  const notice = document.createElement("div");
  notice.className = "yesterday-answers-pending";
  notice.textContent = "Yesterday's most popular answers are still being tallied. Check back shortly!";
  ...
  return contestants
    ...
    .map(c => {
      ...
      return {
        ...
        // Deliberately not part of searchLabel: aliases are nicknames the
        // dropdown accepts but never shows, so they stay out of the text that
        // gets rendered and out of the value the input is filled with.
        normalizedSearchAliases: getSearchAliases(c).map(normalizeAutocompleteText),
...
/** Nicknames a castaway can be found by that are in none of their names. */
function getSearchAliases(c) {
  return (Array.isArray(c?.search_aliases) ? c.search_aliases : []).filter(Boolean);
}

/** Whether a query matches the castaway's names or any of their aliases. */
function matchesSearchQuery(entry, query) {
  if (entry.normalizedSearchLabel.includes(query)) return true;
  return entry.normalizedSearchAliases.some(alias => alias.includes(query));
}
...
function getAutocompleteMatchBonus(entry, query) {
  ...
  // Only a query that opens an alias scores. Matching one mid-string would mean
  // "tom" scoring off "big tom" and lifting Tom Buchanan above the other four
  // Toms on a query that has nothing to do with his nickname -- the alias is
  // here to make "big tom" work, not to re-rank plain names.
  const aliases = getSearchAliases(entry.contestant).map(normalizeAutocompleteText);
...
function attachAutocomplete(input) {
  ...
  input.addEventListener("input", () => {
    ...
    const matches = names
      .filter(n => matchesSearchQuery(n, value))
      .map(entry => {
...
function showValidContestantsPopup(r, c) {
  ...
  function renderList() {
    ...
    sorted.forEach(({ contestant, name, rarityScore }) => {
      const nameItem = document.createElement("div");
      nameItem.className = "modal-list-item";

      appendAnswerIdentity(nameItem, contestant, name);
  ...
  mountModalDialog(modal, modalContent, [title, sortSwitch, modalList]);
}

/**
 * Wire up a modal's shell: mount it, trap focus inside it, and close it on
 * Escape, on a click outside it, or on its own Close button.
 *
 * Lives apart from any one dialog's content because the postgame grid and
 * yesterday's recap open different lists through the same behaviour, and a
 * second hand-rolled focus trap is a second one to get subtly wrong.
 */
function mountModalDialog(modal, modalContent, children) {
  const closeButton = document.createElement("button");
  closeButton.type = "button";
  closeButton.className = "modal-close";
  closeButton.textContent = "Close";

  modalContent.append(...children, closeButton);
...
// A circular crop centred on the South African flag lands on the widest part
// of the white fimbriation, so the badge reads as mostly white. Zooming in and
// shifting toward the hoist puts the black triangle and gold chevron forward
// instead -- the parts no other flag in the set resembles. Carried as a class
// because the crop belongs to this one asset, whichever element holds it.
function applyFlagBackground(element, asset) {
  const flag = asset || "images/flags/unknown.svg";
  element.style.backgroundImage = `url("${flag}")`;
  if (/\bza\.svg$/.test(flag)) {
    element.classList.add("edition-flag-za");
  }
}
...
function createEditionFlagBadge(config) {
  ...
  if (!config?.split) {
    applyFlagBackground(badge, config?.flag);
    return badge;
  }
  ...
  applyFlagBackground(topLeftHalf, config.topLeftFlag);
  ...
  applyFlagBackground(bottomRightHalf, config.bottomRightFlag);
