...
const TRIBE_COLOR_HEX_OVERRIDES = {
  ...
  // Sits in the 330-340 overlap with Araw and Calena, but unlike them it reads
  // as pink alone rather than as a red that happens to be pink too.
  "#E60066": ["pink"], // Rebels (AU09)
  // #333333 is dark enough to read as black by eye but sits at 20% lightness,
  // just above the l <= 0.1 cutoff the black branch uses, so it needs saying
  // explicitly.
  "#333333": ["black"] // Lewatu (US49)
};
...
function normalizeColors(hex) {
  ...
  // Pink runs to 340, deliberately overlapping red's 330 floor: the hot pinks
  // between the two (Araw at 332, Calena at 337) read as both, and the band
  // used to stop short of them and call them red alone. Above 340 the hues are
  // crimsons -- Xhakum at 348 is the nearest -- and stay red only.
  if (h >= 315 && h < 340) families.add("pink");
